...
class PostFirestoreRepository(private val db: FirebaseFirestore) : PostRepository {
    ...
    private fun documentToPost(document: DocumentSnapshot): Post {
        ...
        val tags = document.get("tags") as? List<EveryTag> ?: throw IllegalArgumentException()
        @Suppress("UNCHECKED_CAST")
        val paymentMethods =
            document.get("paymentMethods") as? List<PaymentMethod>
        ...
        val status = document.get("status") as? PostStatus ?: throw IllegalArgumentException()
        @Suppress("UNCHECKED_CAST")
        val media = document.get("media") as? List<String> ?: throw IllegalArgumentException()
        val postType = document.get("type") as? PostType ?: throw IllegalArgumentException()
