...
class NewRequestViewModel(
      ...
      fun setDescription(newDescription: String) {
          _uiState.update {
                it.copy(
                    ...
                    descriptionError = if (newDescription.isBlank()) "Description cannot be empty" else ""
                )
            }
      }
    ...
    fun removeTag(tag: EveryTag) {
      ...
      fun togglePaymentMethod(method: PaymentMethod) {
          _uiState.update { current ->
              val newMethods = if (method in current.paymentMethods) {
      ...
      viewModelScope.launch {
          ...
          try {
              ...
              val request = Request(
                  ...
                  expiry = Timestamp(Date(System.currentTimeMillis() + 30L * 24 * 60 * 60 * 1000)), // DUMMY
              ...
              _uiState.update {
                  it.copy(
                      isLoading = false,
                      submitError = e.message ?: "Failed to create post"
                  )
              }
          }
      }
